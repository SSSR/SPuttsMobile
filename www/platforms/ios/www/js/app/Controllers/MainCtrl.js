    ...
    $scope.postMessageToNewsFeed = function () {
        ...
        $http.post(socialputtsLink + "/api/News/AddPost?userId=" + $.jStorage.get('user').userId + "&isFromManageResponsesPage=true", model)
            .success(function () {
                ...
                getAllPostAndComment(1);
                /* alert("Message has been posted to News Feed!");#1#
            });
    };
...
/*.controller('AccountCtrl', function ($scope, $http, $location, $route) {
    ...
    $scope.login = function () {
       ...
       //
        ...
        var fbLoginSuccess = function(response) {
            ...
            //  { fields: "name,first_name,last_name,picture,email" },
            facebookConnectPlugin.api('/me', function(myInfo) {
                    alert("myInfo " + JSON.stringify(myInfo));
                    var city = "";
                    var state = "";
                    bindFbAccountToSp({
    ...
    var publishStoryFriend = function () {
...
/*.controller('ApproveCtrl', function ($scope, $http, $location) {
    ...
    var getFriends = function () {
        ...
        facebookConnectPlugin.api('/me/taggable_friends', { fields: 'id, name, picture' }, function (response) {
            alert('me' + JSON.stringify(response));
            if (response.error) {
                alert(JSON.stringify(response.error));
            } else {
        ...
        //        facebookConnectPlugin.api("/me/taggable_friends", ["id,name,picture"],
        //            function (response) {
        //                alert('me' + JSON.stringify(response));
        //                if (response.error) {
        //                    alert(JSON.stringify(response.error));
    ...
    /*.controller('FbPostCtrl', function ($scope, $http, $location) {
        var facebookWallPost = function () {
            ...
            //            });
            //encodeURIComponent('Testing 123')
